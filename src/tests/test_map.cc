...
class MapTest : public ::testing::Test {
 protected:
  ...
  std::map<int, std::string> std_map{
      ...
      p14, p15, p16, p17, p18, p19, p20, p21, p22, p23, p24, p25, p26,
      p27, p28, p29, p30, p31, p32, p33, p34, p35, p36, p37};
  void eq_map(mystd::map<int, std::string> map,
              std::map<int, std::string> std_map);
};

void MapTest::eq_map(mystd::map<int, std::string> map,
                     std::map<int, std::string> std_map) {
  EXPECT_EQ(map.size(), std_map.size());
  mystd::map<int, std::string>::iterator i1 = map.begin();
...
TEST_F(MapTest, copy_constructor) {
  mystd::map<int, std::string> copy(map);
  eq_map(copy, std_map);
}

TEST_F(MapTest, move_constructor_assignment) {
  mystd::map<int, std::string> move1(std::move(map));
  eq_map(move1, std_map);
  mystd::map<int, std::string> move2 = std::move(move1);
...
TEST_F(MapTest, insert) {
  std::pair<mystd::map<int, std::string>::iterator, bool> res =
...
TEST_F(MapTest, swap) {
  mystd::map<int, std::string> empty;
...
TEST_F(MapTest, merge) {
  ...
  mystd::map<int, std::string> map2{pair1, pair2, pair3, pair4, pair5,
